...
module.exports = {
    ...
    updateItemCategory: async (req, res, next) => {
        const body = req.body;
        await ItemCategory.updateOne({ _id: body._id }, body).then(result => {
    ...
    getItemCategoryByCompany: async (req, res) => {
        const { company_id } = req.body;
        await ItemCategory.find({ is_active: true, company_id: company_id }, { is_active: 0, company_id: 0 }).then(result => {
            if (!result)
    ...
    deleteItemCategory: async (req, res) => {
        ...
        await ItemCategory.deleteOne({ _id: Object(id) }).then(result => {
